...
while ($row = mysqli_fetch_assoc($result)) {
    $env[$row['key_name']] = $row['key_value'];
}

$LIEFERSOFT_COMPANY_ID = $env['liefersoft_company_key'] ?? '';
$LIEFERSOFT_LOGIN      = $env['liefersoft_login_key'] ?? '';
$LIEFERSOFT_PASSWORD   = $env['liefersoft_password_key'] ?? '';

// Stop script if any credential is missing
if (empty($LIEFERSOFT_COMPANY_ID) || empty($LIEFERSOFT_LOGIN) || empty($LIEFERSOFT_PASSWORD)) {
    header('Content-Type: application/json');
    echo json_encode([
        "status" => false,
        "message" => "Liefersoft credentials are missing in environment settings."
    ]);
    exit;
}
