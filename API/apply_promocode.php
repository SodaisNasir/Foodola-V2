...
if ($eligible_users_date) {
    if ($registration_date < $eligible_users_date) {
        echo json_encode(["status" => "error", "message" => "This promo code is only for new users"]);
        exit;
    }
}
...
if ($min_order && $total_orders < $min_order) {
    echo json_encode([
        "status" => "error",
        "message" => "You must have at least $min_order completed orders to use this promo code"
    ]);
    exit;
}
...
if (mysqli_num_rows($usage_result) > 0) {
    echo json_encode(["status" => "error", "message" => "You have already used this promo code today"]);
    exit;
}
...
if (mysqli_query($conn, $update_sql)) {
    ...
    if (mysqli_query($conn, $insert_usage)) {
        ...
        echo json_encode(["status" => "success", "message" => "Promo code applied successfully", "amount_added" => $promo_value]);
    } else {
        echo json_encode(["status" => "error", "message" => "Failed to store promo code usage"]);
    }
} else {
