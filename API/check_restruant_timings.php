...
if ($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC') {
    ...
    // 2. Check holiday timings table first
    $holiday_sql = "SELECT * FROM holiday_timings WHERE date = '$today_date'";
    $holiday_res = mysqli_query($conn, $holiday_sql);

    if ($holiday_res && mysqli_num_rows($holiday_res) > 0) {
        $holiday = mysqli_fetch_assoc($holiday_res);
        ...
        if (
            ...
            echo json_encode([
                "status" => true,
                "Response_code" => 200,
                "Message" => "Das Restaurant ist heute (Feiertag) geöffnet.",
                "english_message" => "Restaurant is open today (holiday)."
            ]);
        } else {
            echo json_encode([
                "status" => false,
                "Response_code" => 203,
                "Message" => "Das Restaurant ist heute (Feiertag) zu dieser Zeit geschlossen.",
                "english_message" => "Restaurant is closed now (holiday)."
            ]);
        }
        exit;
    }

    // 3. Fall back to regular working hours
    ...
    if ($data) {
        ...
        if (($current_time >= $start1 && $current_time <= $end1) ||($current_time >= $start2 && $current_time <= $end2)) {
            ...
            // Predict next opening time
            if ($current_time < $start1) {
                $nextOpen = $start1;
                $msg = "Das Restaurant soll heute um $nextOpen eröffnet werden.";
                $english_msg = "Restaurant shall be opened today at $nextOpen.";
            } elseif ($current_time > $end1 && $current_time < $start2) {
                $nextOpen = $start2;
                $msg = "Das Restaurant wird später heute um $nextOpen geöffnet.";
                $english_msg = "Restaurant will open later today at $nextOpen.";
            } else {
                $next_day = date('l', strtotime('+1 day'));
                $next_sql = "SELECT `start_time_1` FROM `tbl_working_hours` WHERE `day` = '$next_day'";
                $next_exe = mysqli_query($conn, $next_sql);
                $next_data = mysqli_fetch_array($next_exe);
                $nextOpen = $next_data['start_time_1'] ?? 'unbekannt';
                $msg = "Das Restaurant wird morgen um $nextOpen geöffnet.";
                $english_msg = "Restaurant shall open tomorrow at $nextOpen.";
            }
        ...
        echo json_encode([
            "status" => false,
            "Response_code" => 500,
            "Message" => "Für heute sind keine Arbeitszeiten festgelegt.",
            "english_message" => "No working hours set for today."
        ]);
    }
